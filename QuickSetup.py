...
class PipInstallerApp:
    def __init__(self, root):
        ...
        self.examples_download_urls = {
            ...
            "Gitee": "https://gitee.com/OpenWSGR/AutoWSGR-examples/repository/archive/main.zip"
        }
    ...
    def create_widgets(self):
        ...
        ttk.Label(self.root, text="镜像源:").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        ...
        self.mirror_combobox.grid(row=2, column=1, padx=5, pady=5)
        self.mirror_combobox.bind("<<ComboboxSelected>>", self.save_mirror_config)
        ttk.Button(self.root, text="安装", command=self.install_package).grid(row=2, column=2, padx=5, pady=5)
        ttk.Button(self.root, text="更新", command=self.update_package).grid(row=2, column=3, padx=5, pady=5)
        ...
        ttk.Button(self.root, text="清空输出", command=self.clear_output_area).grid(row=5, column=0, padx=5, pady=5)
